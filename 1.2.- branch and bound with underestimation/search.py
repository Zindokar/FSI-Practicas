...
def tree_search(problem, fringe):
    ...
    while fringe:
        ...
        if problem.goal_test(node.state):
            printf ("Node count: %d ", fringe.nodeCount)
            return node
...
def branch_and_bound_with_underestimation_tree_search(problem):
    """Search the cheapest movement from current node to the next node."""
    return tree_search(problem, SortedListByCostAndHeuristic(problem))  # SortedListByCost -> fringe


def branch_and_bound_tree_search(problem):
    """Search the cheapest movement from current node to the next node."""
    return tree_search(problem, SortedListByCost())  # SortedListByCost -> fringe
